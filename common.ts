...
import { readableStreamToString, AblyError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new AblyError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new AblyError(e, e.response?.data)
                }
                if (innerError instanceof AblyError) {
                    // Got "AblyError" from the above try block
                    throw innerError;
                }
